...
impl Pipeline {
    ...
    pub fn new(issue_number: u64, repo: String, runs_dir: &Path, repos_dir: &Path) -> Self {
        ...
        let branch_name = format!("familiar/issue-{}", issue_number);
    ...
    pub async fn do_understand(&mut self) -> Result<bool> {
        ...
        let learnings_path = self.worktree.join(".familiar").join("learnings.md");
        ...
        summary.push_str("\n## Repo Learnings\n");
        if learnings.is_empty() {
            summary.push_str("- (no .familiar/learnings.md found)\n");
        } else {
    ...
    pub async fn do_implement(&mut self, config: &Config) -> Result<bool> {
        ...
        let worktree_learnings = self.worktree.join(".familiar").join("learnings.md");
    ...
    pub async fn do_submit(&mut self) -> Result<bool> {
        ...
        let commit_msg = format!("familiar: implement issue #{}", self.issue_number);
            ...
            .context("Submit: failed to check for existing PR")?
        {
            ...
            let pr_title = if self.issue_title.is_empty() {
                format!("familiar: issue #{}", self.issue_number)
            } else {
                format!("familiar: {} (#{})", self.issue_title, self.issue_number)
            };
    ...
    pub async fn do_watch(&mut self) -> Result<bool> {
        ...
        // Collect @familiar-mentioned comments (regular PR comments that summon the agent).
        let familiar_comments: Vec<&github::PrComment> = status
            .comments
            .iter()
            .filter(|c| {
                let login = &c.author.login;
                let is_human = !login.ends_with("[bot]") && login != "github-actions";
                is_human && c.body.contains("@familiar")
            })
            .collect();

        // React with 👀 on each @familiar comment to acknowledge it.
        for c in &familiar_comments {
    ...
    pub async fn do_fix(&mut self, config: &Config) -> Result<bool> {
        ...
        // IMPLEMENT or a previous FIX agent appended to .familiar/learnings.md.
        let worktree_learnings = self.worktree.join(".familiar").join("learnings.md");
        let learnings = if worktree_learnings.exists() {
            read_file_or(&worktree_learnings, "")
        } else {
        ...
        github::commit_all(&self.worktree, "familiar: fix blockers")
            .await
...
mod tests {
    ...
    fn test_load_agent_prompt_substitutes_placeholders() {
        let dir = std::env::temp_dir().join("familiar_test_agents");
    ...
    fn test_load_agent_prompt_missing_file() {
        let dir = std::env::temp_dir().join("familiar_test_agents_missing");
    ...
    fn test_load_agent_prompt_no_vars() {
        let dir = std::env::temp_dir().join("familiar_test_agents_novars");
        let _ = fs::create_dir_all(&dir);
    ...
    fn test_load_agent_prompt_multiple_occurrences() {
        let dir = std::env::temp_dir().join("familiar_test_agents_multi");
        let _ = fs::create_dir_all(&dir);
    ...
    #[test]
    fn test_branch_name_uses_familiar_prefix() {
        let tmp = std::env::temp_dir().join("familiar_test_branch_name");
        let runs_dir = tmp.join("runs");
        let repos_dir = tmp.join("repos");
        let _ = fs::create_dir_all(&runs_dir);
        let _ = fs::create_dir_all(&repos_dir);

        let p = Pipeline::new(42, "owner/repo".to_string(), &runs_dir, &repos_dir);
        assert_eq!(p.branch_name, "familiar/issue-42");
    ...
    fn test_parse_verify_verdict_from_file() {
        let dir = std::env::temp_dir().join("familiar_test_verify_verdict");
        let _ = fs::create_dir_all(&dir);
