...
impl Pipeline {
    ...
    async fn do_submit(&mut self, config: &Config) -> Result<bool> {
            ...
            .context("Submit: failed to check for existing PR")?
        {
            ...
            // Try to generate an original PR description via copilot.
            let (problem_summary, solution_summary) =
                match generate_pr_description(config, &self.run_dir, &self.worktree).await {
                    Ok((p, s)) => (p, s),
                    Err(e) => {
                        tracing::warn!(
                            "Failed to generate PR description via copilot, falling back: {:#}",
                            e
                        );
                        // Fallback: truncated issue body + plan summary.
                        let issue_body = read_file_or(&self.run_dir.join("issue_body.md"), "");
                        let plan = read_file_or(&self.worktree.join("plan.md"), "");
                        (
                            truncate_to_sentences(&issue_body, 3),
                            extract_plan_summary(&plan),
                        )
                    }
                };
...
/// Ask copilot to write an original PR description based on the issue, plan,
/// and diff. Returns (problem, solution) text. The agent is instructed to write
/// in its own words — not copy from the issue.
async fn generate_pr_description(
    config: &Config,
    run_dir: &Path,
    worktree: &Path,
) -> Result<(String, String)> {
    let issue_body = read_file_or(&run_dir.join("issue_body.md"), "(no issue body)");
    let plan = read_file_or(&run_dir.join("plan.md"), "");

    // Get a stat-summary of the diff (limit full diff to avoid huge prompts).
    ...
    // Also grab a truncated patch diff for context (first 200 lines).
    let patch_output = tokio::process::Command::new("git")
        ...
        .and_then(|o| String::from_utf8(o.stdout).ok())
        .unwrap_or_default();
    let truncated_patch: String = patch_output
        .lines()
        .take(200)
        .collect::<Vec<_>>()
        .join("\n");

    let pr_desc_path = run_dir.join("pr_description.md");

    let prompt = format!(
        "You are a technical writer summarizing a code change for a pull request.\n\
         \n\
         ## GitHub Issue (for context only — do NOT copy this text)\n\
         {issue_body}\n\
         \n\
         ## Implementation Plan\n\
         {plan}\n\
         ...
         \n\
         ## Instructions\n\
         Write a file at `{pr_desc_path}` with exactly two markdown sections:\n\
         \n\
         ### Format\n\
         ```\n\
         ## Problem\n\
         \n\
         <max 3 sentences. Write an ORIGINAL problem statement in your own words.\n\
         Do NOT copy or paraphrase the issue description. Explain WHAT was wrong\n\
         or missing from a user/developer perspective.>\n\
         \n\
         ## Solution\n\
         \n\
         <A short description (3-5 sentences) of HOW you solved the problem.\n\
         Mention specific files changed, new functions added, or architectural\n\
         decisions made. Be concrete.>\n\
         ```\n\
         \n\
         Write ONLY the file. No other output.\n",
        issue_body = issue_body,
        plan = plan,
        diff_output = diff_output,
        truncated_patch = truncated_patch,
        pr_desc_path = pr_desc_path.display(),
    );

    let prompt_path = run_dir.join("prompt_submit.md");
    fs::write(&prompt_path, &prompt).context("Submit: failed to write prompt_submit.md")?;
    ...
    .await
    .context("Submit: copilot PR-description generation failed")?;

    // Parse the generated file.
    let content = tokio::fs::read_to_string(&pr_desc_path)
        .await
        .context("Submit: failed to read generated pr_description.md")?;

    let (problem, solution) = parse_pr_description(&content);

    if problem.is_empty() && solution.is_empty() {
        anyhow::bail!("pr_description.md was empty or had no recognizable sections");
    }

    Ok((problem, solution))
}

/// Parse a markdown file with `## Problem` and `## Solution` sections.
/// Returns (problem_text, solution_text).
fn parse_pr_description(content: &str) -> (String, String) {
    ...
    for line in content.lines() {
        ...
        if lower.starts_with("## problem") {
            current_section = Some("problem");
            continue;
        } else if lower.starts_with("## solution") {
            current_section = Some("solution");
            continue;
...
/// Return the first `n` sentences from `text`, or the full text if shorter.
fn truncate_to_sentences(text: &str, n: usize) -> String {
    ...
    for (i, ch) in trimmed.char_indices() {
        if ch == '.' || ch == '!' || ch == '?' {
            // Only count as a sentence boundary if followed by whitespace or end-of-string.
            let next = trimmed[i + ch.len_utf8()..].chars().next();
...
/// Extract a short summary from plan.md content.
/// Uses the first paragraph, or the "Summary" section if present.
fn extract_plan_summary(plan: &str) -> String {
    ...
    for line in trimmed.lines() {
        ...
        if lower.starts_with("# summary") || lower.starts_with("## summary") {
            // Collect lines until the next heading or blank section.
            let start = trimmed.find(line).unwrap() + line.len();
            let rest = trimmed[start..].trim_start();
            let mut para = String::new();
            for l in rest.lines() {
                if l.starts_with('#') || (l.trim().is_empty() && !para.is_empty()) {
                    break;
                }
                if !l.trim().is_empty() {
                    if !para.is_empty() {
...
mod tests {
    ...
    #[test]
    fn parse_pr_description_both_sections() {
        let content = "\
## Problem

The PR description was just copying the issue text verbatim.
Users saw duplicated content with no original insight.

## Solution

Added a copilot call in `do_submit()` that generates an original
problem statement and solution description from the diff and plan.
";
        let (problem, solution) = parse_pr_description(content);
        assert!(problem.contains("copying the issue text"));
        assert!(problem.contains("no original insight"));
        assert!(solution.contains("copilot call"));
        assert!(solution.contains("do_submit"));
    }

    #[test]
    fn parse_pr_description_only_problem() {
        let content = "## Problem\n\nSomething was broken.\n";
        let (problem, solution) = parse_pr_description(content);
        assert_eq!(problem, "Something was broken.");
        assert!(solution.is_empty());
    }

    #[test]
    fn parse_pr_description_only_solution() {
        let content = "## Solution\n\nFixed it by adding a check.\n";
        let (problem, solution) = parse_pr_description(content);
        assert!(problem.is_empty());
        assert_eq!(solution, "Fixed it by adding a check.");
    }

    #[test]
    fn parse_pr_description_empty() {
        let (problem, solution) = parse_pr_description("");
        assert!(problem.is_empty());
        assert!(solution.is_empty());
    }

    #[test]
    fn parse_pr_description_stops_at_unknown_section() {
        let content = "\
## Problem
...
";
        let (problem, solution) = parse_pr_description(content);
        assert_eq!(problem, "First sentence.");
        assert_eq!(solution, "The fix.");
    }

    #[test]
    fn parse_pr_description_case_insensitive_headers() {
        let content = "## PROBLEM\n\nUpper case header.\n\n## SOLUTION\n\nAlso upper.\n";
        let (problem, solution) = parse_pr_description(content);
        assert_eq!(problem, "Upper case header.");
        assert_eq!(solution, "Also upper.");
    }
}
